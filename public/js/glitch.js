window.addEventListener("DOMContentLoaded",()=>{
    ...
    setInterval(() => {
        ...
        if (width >= 1440) {
            for (let i = 0; i < glitchEffect.length; i++) {
                glitchEffect[i].style.width = Math.floor(Math.random() * (140 - 115) + 115) + 'px';
                glitchEffect[i].style.height = Math.floor(Math.random() * (50 - 45) + 45) + 'px';
            }
        } else if (width >= 1520) {
            for (let i = 0; i < glitchEffect.length; i++) {
                glitchEffect[i].style.width = Math.floor(Math.random() * (140 - 115) + 115) + 'px';
                glitchEffect[i].style.height = Math.floor(Math.random() * (55 - 50) + 50) + 'px';
            }
        } else {
            for (let i = 0; i < glitchEffect.length; i++) {
                glitchEffect[i].style.width = Math.floor(Math.random() * (85 - 75) + 75) + 'px';
                glitchEffect[i].style.height = Math.floor(Math.random() * (40 - 30) + 30) + 'px';
            }
        } 
    }, 120);
